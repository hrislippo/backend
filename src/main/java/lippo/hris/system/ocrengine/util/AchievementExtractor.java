...
@Component
public class AchievementExtractor {
    public SkillsCoordinateResponse coordinate(List<List<TextPosition>> positionList) {
        ...
        for(List<TextPosition> textPositions : positionList) {
            ...
            for (Integer y : sortedY) {
                ...
                if(word.toString().toLowerCase().contains("achievement")){
                    Integer startIndex = word.toString().toLowerCase().indexOf("achievement");
                    startPositions.add(wordPositions.get(startIndex));

                    Integer endIndex = startIndex + "achievement".length() - 1;
